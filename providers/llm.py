...
Local Ollama by default (no cost, no key). Swap to Groq, OpenAI, together.ai,
etc. later by editing LLM_BASE_URL / LLM_API_KEY / LLM_MODEL in .env — no
code changes. This is the only place in the pipeline that talks to the LLM;
every worker imports `chat` or `chat_json`.
...
def chat_json(messages, **kwargs) -> str:
    ...
    Use for extraction, ranking, and critic steps where you want
